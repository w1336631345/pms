...
public interface RoomUsageDao extends BaseDao<RoomUsage> {
	...
	@Query(value = "select a.* from  t_room_usage a,t_guest_room b where a.guest_room_id = b.id and b.guest_room_id =?1 and end_date_time >=?2 order by start_date_time desc limit 1", nativeQuery = true)
