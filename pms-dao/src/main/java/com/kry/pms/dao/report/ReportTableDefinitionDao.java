...
public interface ReportTableDefinitionDao extends BaseDao<ReportTableDefinition> {
    @Query(value = "select new com.kry.pms.model.http.response.report.ReportTableDefinitionListVo(id,name,groupKey,type) from ReportTableDefinition where groupKey=?1 and deleted = 0")
