...
//		List<Map<String, Object>> totle = roomTypeQuantityDao.getByTimeAndRoomType(hotelCode, times, roomTypeId);
	...
	@Override
	public List<Map<String, Object>> getByTimeAndRoomType2(String hotelCode, String times, String[] buildIds, String[] roomTypeIds) {
		...
		for(int i=0; i<list.size(); i++){
			...
			for(int j=0; j<schmes.size(); j++){
				...
				if(fId.equals(id)){
					...
					newMap.put("price_"+typeCode, price);
					newMap.put("typeName_"+typeCode, roomTypeName);
					Map<String, Object> tMap = roomTypeQuantityDao.mapByTimeAndRoomType2(hotelCode, times, bIds, room_type_id);
